<template>
  <div>
    ...
    <el-card>
      ...
      <!-- tab页签区域 -->
      <el-tabs v-model="activeName" @tab-click="hanTabClick">
        <!-- 添加动态参数的面板 -->
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(attr_id) {
      const { data: res } = await this.$http.get(`categories/${this.selectedCateKeys[2]}/attributes/${attr_id}`, { params: { attr_sel: this.activeName } })
      if (res.meta.status !== 200) {
    ...
    async removeParams(attr_id) {
      ...
      const { data: res } = await this.$http.delete(`categories/${this.selectedCateKeys[2]}/attributes/${attr_id}`)
      if (res.meta.status !== 200) {
    ...
    showInput(row) {
      ...
      // 让文本框自动获得焦点
      // $nextTick方法的作用，就是当页面上元素被重新渲染之后，才会指定回调函数
      this.$nextTick((_) => {
        // 第一个ref拿到el-input第二个ref从input身上拿到原生DOM节点
